	...
	[
		{
			"form_date" : "2024.06",
			"to_date" : "2025.01",
			"project_name" : "AI实时交互虚拟数字人系统",
			"position" : "资深Java工程师",
			"description" : "本项目是一个高性能、高可用的AI虚拟数字人实时交互系统。项目核心是一个具备自然语言对话、情感化表情与动作的3D虚拟形象，用户可通过语音或文字与数字人进行沉浸式互动。系统采用C/S架构，我主要负责整个服务端的架构设计、开发与运维工作。客户端使用 Unity 引擎，负责 3D 数字人的渲染、动画驱动、音频播放及用户输入采集。服务端采用Java技术栈。",
			"function_list": {
			...
			"performance": [
				...
				"系统稳定性：设计并实现了服务熔断与降级机制（基于 Sentinel ）。当第三方 TTS 服务响应缓慢时，自动降级为仅返回文本信息，保障了核心对话功能的可用性，系统整体 SLA 提升至99.95%。",
				"成本与效率：通过连接池化管理和异步化处理（MQ）对第三方 AI API 的调用，减少了不必要的连接开销，并将 TTS 合成等耗时任务异步化，提升了请求响应速度，预计每月节省约15%的云服务调用成本。",
				"完整交付：独立负责了从技术选型、架构设计、数据库设计到核心编码、单元测试、压力测试及最终上线部署的全流程，保证了项目的成功落地和稳定运行"
			]
		},
		{
			"form_date" : "2023.02",
